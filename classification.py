...
Original file is located at
    https://colab.research.google.com/drive/1vB3wcwkLbJVrk-rzwIppgIfnqjSNJYce
"""
...
[link text](https://)Another common type of text classification is sentiment analysis, which (as we saw in <<chapter_introduction>>) aims to identify the polarity of a given text. For example, a company like Tesla might analyze Twitter posts like the one in <<tesla-tweet>> to determine whether people like its new car roofs or not.
...
model_name = f"{model_ckpt}-newsmodelclassification"
...
# tokenizer = AutoTokenizer.from_pretrained("aatmasidha/distilbert-base-uncased-finetuned-emotion")

# model = AutoModelForSequenceClassification.from_pretrained("aatmasidha/distilbert-base-uncased-finetuned-emotion")
